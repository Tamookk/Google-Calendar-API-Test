...
# If modifying this, delete file 'token.pickle'
...
def main(offset):
    """
    Fetches and prints out calendar events for all calendars associated with a given Google account.
    Events are printed in order, grouped by date.

    offset:     this parameter is the day offset, i.e., how many days ahead to grab events.
    """
    creds = None
    ...
    tomorrow = (now + datetime.timedelta(days=offset)).replace(hour=0, minute=0, second=0, microsecond=0)
    # Create events list
    events_list = []
    ...
    for key in calendar_ids:
        ...
        event_result = service.events().list(calendarId=calendar_ids[key], timeMin=now.isoformat() + localoffset,
                                             timeMax=tomorrow.isoformat() + localoffset, maxResults=10,
    ...
    for item in grouped_events:
        ...
        for event in item[1]:
            try:
                ...
                if (event[1].year == event[2].year) and (event[1].month == event[2].month) and (
                        event[1].day == event[2].day):
...
if __name__ == '__main__':
    offset = input("How many days ahead do you want to see?")
    main(int(offset))
